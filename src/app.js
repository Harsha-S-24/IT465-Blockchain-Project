...
App = {
    ...
    loadAccount: async () => {
        ...
        var shortenedAccount = account.substring(0, 10) + "..";
        document.getElementById("metamask").innerHTML = shortenedAccount;
    },
    ...
    renderCertGrid: async () => {
      // Load the total vaccine count from the blockchain
      const vacc=['Covishield','Covaxin','Sputnik']
      ...
      for(var i = 1; i <= vaccineCount; i++) {
          ...
          $newVaccineTemplate.find('.vaccinee').html(vacc[vaccine-1]);
          var qr = new QRious({
            value: tx_hash
          });

          //$newVaccineTemplate.attr('onClick', "App.generatePDF('"+name+"','"+aadhar+"','"+qr.toDataURL() +",'"+vaccine+"')");
          $newVaccineTemplate.attr('onClick', `App.generatePDF('${name}','${aadhar}','${qr.toDataURL()}','${vacc[vaccine-1]}')`);
  ...
  receiveVaccine: async () => {
    ...
    const location = $('#locationDropdownList').val();
    const vaccine = $('#vaccineDropdownList').val();
    const temp = $('#temp').val();
    var perf=0
  
    // Form validation
    if (!name || !aadhar || !location || !vaccine || !temp) {
    ...
    // Aadhaar Card Number validation using regex
    ...
    // Proceed with receiving vaccine
    if(perf==0) {
    try {
      App.setLoading(true);
      ...
      var h = await App.vaccineDistr.receiveVaccine.sendTransaction(certId, name, aadhar, ts, vaccine);
  ...
  ,
    ...
    setLoading: (boolean) => {
     ...
     generatePDF: async (name,aadhar,qrURL,vaccine) => {
    ...
    const SanChezFont  = await pdfDoc.embedFont(fontBytes);
     ...
     firstPage.drawText("Aadhar Number: "+aadhar, {
      x: 270,
      y: 230,
      size: 25,
      font: SanChezFont ,
    ...
    firstPage.drawText("Dose: "+vaccine, {
      x: 270,
      y: 190,
      size: 25,
      font: SanChezFont ,
